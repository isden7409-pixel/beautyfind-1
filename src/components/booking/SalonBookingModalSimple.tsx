/**
 * Модальное окно резервации для салона БЕЗ мастеров
 * Простая резервация: Услуга → Дата → Время → Подтверждение
 */
...
interface SalonBookingModalSimpleProps {
  salon: Salon;
  isOpen: boolean;
  onClose: () => void;
...
type Step = 'service' | 'date' | 'time' | 'confirmation' | 'success';

export const SalonBookingModalSimple: React.FC<SalonBookingModalSimpleProps> = ({
  salon,
  isOpen,
  onClose,
  onBookingComplete,
  ...
  const t = {
    cs: {
      ...
      stepService: 'Krok 1 ze 4: Vyberte službu',
      stepDate: 'Krok 2 ze 4: Vyberte datum',
      stepTime: 'Krok 3 ze 4: Vyberte čas',
      stepConfirmation: 'Krok 4 ze 4: Potvrzení',
      next: 'Další',
      back: 'Zpět',
      cancel: 'Zrušit',
    ...
    en: {
      title: 'Salon Booking',
      stepService: 'Step 1 of 4: Select service',
      stepDate: 'Step 2 of 4: Select date',
      stepTime: 'Step 3 of 4: Select time',
      stepConfirmation: 'Step 4 of 4: Confirmation',
      next: 'Next',
      back: 'Back',
      cancel: 'Cancel',
  ...
  const loadServices = async () => {
    try {
      const data = await getServices(salon.id, 'salon');
      setServices(data);
    } catch (err: any) {
        ...
        <div className="p-6">
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between text-xs text-gray-500 mb-2">
              <span className={step === 'service' ? 'text-pink-600 font-medium' : ''}>1</span>
              <span className={step === 'date' ? 'text-pink-600 font-medium' : ''}>2</span>
              <span className={step === 'time' ? 'text-pink-600 font-medium' : ''}>3</span>
              <span className={step === 'confirmation' ? 'text-pink-600 font-medium' : ''}>4</span>
            </div>
