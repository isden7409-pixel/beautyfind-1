...
exports.updateAnalyticsOnStatusChange = exports.updateAnalyticsOnBooking = exports.autoCompleteBookings = exports.sendBookingReminders = exports.onBookingCreated = void 0;
 ...
 */
function getBookingConfirmationEmailForClient(booking, language) {
    const serviceName = language === 'cs'
        ? booking.serviceName_cs
        : (booking.serviceName_en || booking.serviceName_cs);
    const dateFormatted = new Date(booking.date).toLocaleDateString(language === 'cs' ? 'cs-CZ' : 'en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
    if (language === 'cs') {
        return {
 ...
 */
function getNewBookingEmailForProvider(booking, language) {
    const serviceName = language === 'cs'
        ? booking.serviceName_cs
        : (booking.serviceName_en || booking.serviceName_cs);
    const dateFormatted = new Date(booking.date).toLocaleDateString(language === 'cs' ? 'cs-CZ' : 'en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
 ...
 */
function getBookingReminderEmail(booking, language) {
    const serviceName = language === 'cs'
        ? booking.serviceName_cs
        : (booking.serviceName_en || booking.serviceName_cs);
    const dateFormatted = new Date(booking.date).toLocaleDateString(language === 'cs' ? 'cs-CZ' : 'en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
 ...
 */
exports.updateAnalyticsOnBooking = functions.firestore
    .document('bookings/{bookingId}')
    .onCreate(async (snap, context) => {
    const booking = snap.data();
    try {
        const providerRef = admin.firestore().collection(booking.providerType === 'salon' ? 'salons' : 'masters').doc(booking.providerId);
 ...
 */
exports.updateAnalyticsOnStatusChange = functions.firestore
    .document('bookings/{bookingId}')
    ...
    try {
        const providerRef = admin.firestore().collection(after.providerType === 'salon' ? 'salons' : 'masters').doc(after.providerId);
        const updates = {
            'analytics.lastUpdated': admin.firestore.FieldValue.serverTimestamp(),
        };
